...
exports.connect = function(socket, data, cb) {
  ...
  function queryAndEmitBuffers(buffer) {
    ...
    client.send(query, function(buffers) {
      ...
      var servers = buffers.filter(function(buffer) {
      ...
      servers.forEach(function(server) {
        socket.emit('servers:create', server);
      });
      buffers.forEach(function(buffer) {
        var server = servers.filter(function(server) {
          return server.title == buffer.server;
        })[0];
        if (server) {
          socket.emit('server/' + server.id + ':createBuffer', buffer);
        }
      });
    });
  }
