...
var hotlist = 'hdata hotlist:gui_hotlist(*) count,buffer';
...
exports.connect = function(socket, data, cb) {
  ...
  function queryAndEmitBuffers(buffer) {
    ...
    query(buffers, buffer, function(buffers) {
      ...
      query(hotlist, '', function(hotlist) {
        buffers.forEach(function(buffer) {
          var hl = hotlist.filter(function(hl) {
            return hl.buffer === buffer.id;
          });

          if (hl.length > 0) buffer.notifications = hl[0].count[1];
